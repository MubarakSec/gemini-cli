...
export function createLSPServerInstance(
  ...
  async function start(): Promise<void> {
    ...
    try {
      state = 'starting';
      debugLogger.debug(`Starting LSP server instance: ${name}`);
      ...
      debugLogger.debug(`LSP server instance started: ${name}`);
    } catch (error) {
      client.stop().catch(() => {});
      initPromise?.catch(() => {});
      state = 'error';
  ...
  async function stop(): Promise<void> {
    ...
    try {
      ...
      await client.stop();
      state = 'stopped';
      debugLogger.debug(`LSP server instance stopped: ${name}`);
    } catch (error) {
      state = 'error';
      lastError =
        error instanceof Error ? error : new Error(errorMessage(error));
  ...
  async function sendRequest<T>(method: string, params: unknown): Promise<T> {
        ...
        if (
          ...
          debugLogger.debug(
            `LSP request '${method}' to '${name}' got ContentModified error, ` +
              `retrying in ${backoff}ms (attempt ${attempt + 1}/${MAX_RETRIES_FOR_TRANSIENT_ERRORS})…`,
          );
          await delay(backoff);
          continue;
        }
    ...
    debugLogger.error(requestError);
    throw requestError;
  }
  ...
  async function sendNotification(
    ...
    try {
      ...
      debugLogger.error(notificationError);
