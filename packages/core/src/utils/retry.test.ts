...
describe('retryWithBackoff', () => {
  ...
  it('should default to 20 maxAttempts if no options are provided', async () => {
    // This function will fail more than 20 times to ensure all retries are used.
    const mockFn = createFailingFunction(25);

    const promise = retryWithBackoff(mockFn);

    await Promise.all([
      expect(promise).rejects.toThrow('Simulated error attempt 20'),
      vi.runAllTimersAsync(),
    ]);
  ...
  it('should default to 20 maxAttempts if options.maxAttempts is undefined', async () => {
    // This function will fail more than 20 times to ensure all retries are used.
    const mockFn = createFailingFunction(25);

    const promise = retryWithBackoff(mockFn, { maxAttempts: undefined });

    // Expect it to fail with the error from the 20th attempt.
    await Promise.all([
      expect(promise).rejects.toThrow('Simulated error attempt 20'),
      vi.runAllTimersAsync(),
    ]);
