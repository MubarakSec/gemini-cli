...
export function formatHoverResult(result: Hover | null, _cwd?: string): string {
  if (!result) {
    return 'No hover information available. This may occur if the cursor is not on a symbol, or if the file is not supported by the LSP server.';
  }
...
export function formatDocumentSymbolResult(
  ...
  const isSymbolInformation =
    firstSymbol && typeof firstSymbol === 'object' && 'location' in firstSymbol;
    ...
    for (const call of calls) {
      ...
      if (call.fromRanges && call.fromRanges.length > 0) {
        const callSites = call.fromRanges
          .map(
            (r: { start: { line: number; character: number } }) =>
